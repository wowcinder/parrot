...
import com.voole.parrot.gwt.ui.shared.logmodel.editor.LogModelEditor;
import com.voole.parrot.gwt.ui.shared.logmodel.editor.LogModelVersionDuplicateEditor;
import com.voole.parrot.gwt.ui.shared.logmodel.editor.LogModelVersionEditor;
import com.voole.parrot.gwt.ui.shared.logmodel.window.LogModelRootColumnTreeGridWindow;
 ...
 * @date 2013年12月26日
 */
public class LogModelTreeGridMenu extends Menu {
	private final LogModelTreeGrid grid;
	...
	public LogModelTreeGridMenu(LogModelTreeGrid grid2) {
		grid = grid2;
	...
	protected void initDelModel() {
		delModel.addSelectionHandler(new SelectionHandler<Item>() {
			...
			public void onSelection(SelectionEvent<Item> event) {
				...
				getGrid().mask("删除中...");
				RpcServiceUtils.LogModelRpcService.deleteModel(model,
	...
	protected void initDelVersion() {
		delVersion.addSelectionHandler(new SelectionHandler<Item>() {
			...
			public void onSelection(SelectionEvent<Item> event) {
				...
				getGrid().mask("删除中...");
				RpcServiceUtils.LogModelRpcService.deleteVersion(verion,
						new RpcAsyncCallback<Void>() {
							@Override
							protected void _onSuccess(Void result) {
	...
	protected void initDuplicateVersion() {
		duplicateVersion.addSelectionHandler(new SelectionHandler<Item>() {
			...
			public void onSelection(SelectionEvent<Item> event) {
				...
				LogModelVersionDuplicateEditor e = new LogModelVersionDuplicateEditor(
						version);
	...
	protected void initEditColumns() {
		editColumns.addSelectionHandler(new SelectionHandler<Item>() {
			@Override
			public void onSelection(SelectionEvent<Item> event) {
				LogModelRootColumnTreeGridWindow w = new LogModelRootColumnTreeGridWindow(
						getSeletedVersion());
				w.show();
			}
		});
	...
	public LogModelTreeGrid getGrid() {
		return grid;
	}
	...
	private LogModelEditor modelEditor;

	public LogModelEditor getModelEditor() {
		if (modelEditor == null) {
			modelEditor = new LogModelEditor();
		}
		return modelEditor;
	}

	private LogModelVersionEditor versionEditor;

	public LogModelVersionEditor getVersionEditor() {
		if (versionEditor == null) {
			versionEditor = new LogModelVersionEditor();
		}
