...
import com.voole.parrot.gwt.common.shared.gridcolumn.LogModelColumnColumnConfig;
...
public class LogModelRootColumnTreeGrid extends
	...
	static {
		...
		name = LogModelColumnColumnConfig.name();
		...
		l.add(name);
		l.add(LogModelColumnColumnConfig.desc());
		l.add(type);
		l.add(table);
		l.add(version);
		l.add(column);
	...
	public LogModelRootColumnTreeGrid(LogModelVersion version) {
		super(new TreeStore<LogModelColumn>(
				PropertyUtils.LogModelColumnProperty.key()), cm, name);
		this.version = version;
		setContextMenu(new LogModelRootColumnTreeGridMenu(this));
