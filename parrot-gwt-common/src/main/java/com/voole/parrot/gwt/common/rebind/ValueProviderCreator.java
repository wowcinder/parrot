 ...
 */
package com.voole.parrot.gwt.common.rebind;
...
public class ValueProviderCreator extends AbstractCreator {
	...
	private final JGenericType valueProviderInterface = getContext()
			.getTypeOracle()
			.findType(Name.getSourceNameForClass(ValueProvider.class))
			.isGenericType();

	private RequiredReadability readability = RequiredReadability.NEITHER;

	public ValueProviderCreator(GeneratorContext ctx, TreeLogger l,
			JMethod valueProviderMethodDecl) {
		super(ctx, l);
		this.supertypeToImplement = valueProviderMethodDecl.getReturnType()
				.isClassOrInterface();
		this.path = generatePath(
				valueProviderMethodDecl.getAnnotation(Path.class),
				valueProviderMethodDecl.getName());
	}

	public ValueProviderCreator(GeneratorContext ctx, TreeLogger l,
			List<String> path, JClassType baseType, JClassType valueType) {
		super(ctx, l);
		this.path = path;
		supertypeToImplement = getContext().getTypeOracle()
				.getParameterizedType(valueProviderInterface,
	...
	protected void create(SourceWriter sw) throws UnableToCompleteException {
		sw.println("public static final %1$s INSTANCE = new %1$s();",
				getSimpleName());
		// @Override
		sw.println("public %1$s getValue(%2$s object) {", getValueTypeName(),
				getObjectTypeName());
		String tempName = "object";
		...
		for (String p : path) {
			if (type == null) {
				getLogger()
						.log(Type.WARN,
								"Trying to find a method in a non-class, non-interface type.");
			}
			// TODO field?
			// TODO is, has
			...
			if (method == null) {
				getLogger().log(Type.WARN,
						"Method get" + cap(p) + " could not be found");
			}
			String newtmepName = "object_" + deep;
			sw.indentln("%1$s %2$s = %4$s.%3$s();", method.getReturnType()
					.getParameterizedQualifiedSourceName(), newtmepName, method
					.getName(), tempName);
			tempName = newtmepName;
			sw.indentln("if(%1$s==null){return null;}", tempName);
			type = method.getReturnType().isClassOrInterface();
			deep++;
		}
		sw.indentln("return %1$s;", tempName);
		sw.println("}");

		// @Override
		sw.println("public void setValue(%1$s object, %2$s value) {",
				getObjectTypeName(), getValueTypeName());
		String setter = getSetterExpression("object", "value");
		if (setter == null) {
			if (readability == RequiredReadability.NEITHER
					|| readability == RequiredReadability.GET) {
				// setter is not required, but log it if someone tries to call
				// it
				getLogger()
						.log(Type.DEBUG,
				...
				sw.indentln("com.google.gwt.core.client.GWT.log(\"Setter was called on "
						+ supertypeToImplement.getName()
						+ ", but no setter exists.\", new RuntimeException());");
			} else {
				getLogger().log(Type.ERROR,
						"No setter can be found, unable to proceed");
	...
	protected JClassType getObjectType() {
		JClassType[] params = ModelUtils.findParameterizationOf(
				valueProviderInterface, supertypeToImplement);
	...
	protected String getSimpleName() {
		return getObjectType().getName().replace('.', '_') + "_"
	 ...
	 * 
	 * @param path
	 *            the path to follow to find the next getter
	 * @param sb
	 *            stringbuild to append the methods to
	 * @return the type returned of the last method
	 * @throws NoSuchMethodException
	 *             if a method cannot be found
	 */
	private JClassType getGetterHelper(List<String> path, StringBuilder sb)
			throws NoSuchMethodException {
		JClassType type = getObjectType();
		for (String p : path) {
			if (type == null) {
	...
	private String getSetterExpression(String objectName, String valueName) {
		...
		if (type == null) {
			getLogger()
					.log(Type.WARN,
							"Trying to find setter method on a non-class, non-interface type ");
			return null;
		}
		String methodName = "set" + cap(path.get(path.size() - 1));
		sb.append(".").append(methodName).append("(").append(valueName)
				.append(")");
